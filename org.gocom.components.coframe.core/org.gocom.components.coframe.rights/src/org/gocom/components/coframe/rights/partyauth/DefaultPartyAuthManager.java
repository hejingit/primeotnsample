 ...
 */
public class DefaultPartyAuthManager extends CoframeDASDaoSupport {
	...
	public CapPartyauth[] getCapPartyauthListByParty(Party party) { // bug: 46048 修改员工信息，修改操作员会产生两条操作员记录
		IDASCriteria criteria = DASManager.createCriteria(CapPartyauth.QNAME);
		criteria.add(ExpressionHelper.eq(IConstants.TENANT_PROPERTY, party.getTenantID()));
		criteria.add(ExpressionHelper.eq("partyId", party.getId()));
		criteria.add(ExpressionHelper.eq("partyType", party.getPartyTypeID()));
		return getDASTemplate().queryEntitiesByCriteriaEntity(CapPartyauth.class, criteria);
	}
}