...
public class OrgHelper {
	...
	/**
	 * 根据父工作组设置子机构部分信息：level, seq, isLeaf,subcount
	 * @param group
	 * @param parentGroup
	 */
	public static void expandGroupPropertyByParent(OrgGroup group, OrgGroup parentGroup){
		if(group == null) return;
		if(parentGroup == null || parentGroup.getGroupid() == null){
			group.setGrouplevel(1);
			//不存在父机构:"."+本机构ID+".";
			group.setGroupseq("."+group.getGroupid()+".");
		}else{
			//级别在父机构的级别上增加1
			group.setGrouplevel(parentGroup.getGrouplevel()+1);
			//存在父机构:父机构顺序+本机构ID+".";
			group.setGroupseq(parentGroup.getGroupseq() + group.getGroupid()+".");
		}
		group.setCreatetime(new Date());
		group.setLastupdate(new Date());
		group.setIsleaf(IOrgConstants.IS_LEAF_YES);
		group.setSubcount(BigDecimal.valueOf(0));
		group.setTenantId(TenantManager.getCurrentTenantID());
	}

	/**
	 * 更改父工作组的信息，isLeaf,subcount
	 * @param parentGroup
	 */
	public static void expandParentGroupProperty(OrgGroup parentGroup) {
		if(parentGroup == null || parentGroup.getGroupid() == null){
			return ;
		}
		parentGroup.setSubcount(parentGroup.getSubcount().add(BigDecimal.valueOf(1)));
		parentGroup.setIsleaf(IOrgConstants.IS_LEAF_NO);
	}
}
