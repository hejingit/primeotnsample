 ...
 */
public class OrgPositionService extends CoframeDASDaoSupport implements IOrgPositionService {
	...
	public QueryPositionEmp[] queryEmployeesOfPosition(String positionid) {
		...
		// 设置查询条件com.eos.das.entity.criteria.CriteriaType
	...
	public int addAuthorizedCriteria(CriteriaType criteriaType){
		...
		for (Party party : orgList) {
			...
			ex.set_op("like");
			ex.set_likeRule("end");
			ex.set("orgOrganization.orgseq", party.getExtAttribute(IConstants.ORG_SEQ));
