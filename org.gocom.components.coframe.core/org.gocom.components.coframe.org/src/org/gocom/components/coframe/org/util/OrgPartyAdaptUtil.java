...
public class OrgPartyAdaptUtil {
	...
	public static Party adapt(OrgOrganization org) {
		...
		//将组织机构的父机构路径保存在扩展属性字段中
		party.putExtAttribute(IConstants.ORG_SEQ, org.getOrgseq());
		return party;
	}
	 ...
	 * @return
	 */
	public static Party adapt(OrgGroup group) {
		if(group == null) return null;
		
		Party party = new Party(IConstants.GROUP_PARTY_TYPE_ID, 
				String.valueOf(group.getGroupid()), group.getGroupname(), 
				group.getGroupname(), group.getTenantId());
		return party;
	}
	...
	public static Party adapt(OrgEmployee emp) {
		...
		party.putExtAttribute(IConstants.EMAIL, emp.getOemail()== null?emp.getPemail():emp.getOemail());
		party.putExtAttribute(IConstants.PEMAIL, emp.getPemail());
		party.putExtAttribute(IConstants.OEMAIL, emp.getOemail());
		party.putExtAttribute(IConstants.MOBILE, emp.getMobileno());
		party.putExtAttribute(IConstants.USER_ID, emp.getUserid());
	 ...
	 * @return
	 */
	public static List<Party> adapt(OrgGroup[] groups) {
		List<Party> returnList = new ArrayList<Party>();
		if (groups != null) {
			for (OrgGroup group : groups) {
				returnList.add(adapt(group));
			}
		}
		return returnList;
	}
	 ...
	 * @return
	 */
	public static List<Party> adaptToList(OrgGroup org) {
		List<Party> returnList = new ArrayList<Party>();
		if (org != null) {
			returnList.add(adapt(org));
		}
