...
import org.gocom.components.coframe.org.IOrgOrganizationService;
...
public class OrgTreeNodeHelper {
	...
	private static IOrgOrganizationService organizationService;
	
	public void setOrganizationService(IOrgOrganizationService orgService) {
	 ...
	 */
	public static DataObject[] buildOrgTreeNodes(OrgOrganization[] orgs, 
			QueryPositionEmp[] positions, OrgEmployee[] emps) {
		List<DataObject> results = new ArrayList<DataObject>();
	...
	public static DataObject[] buildOrgTreeNodes(List<Party> orgPartyList) {
		...
		String[] ids = new String[orgPartyList.size()];
		int num = 0;
		for(Party orgParty : orgPartyList) {
			ids[num++] = orgParty.getId();
		}
		OrgOrganization[] orgs = organizationService.queryOrgOrganizationsByIds(ids);
		
		for(OrgOrganization orgOrganization : orgs) {
			OrgTreeNode node = OrgTreeNode.FACTORY.create();
			node.setNodeId(orgOrganization.getOrgid().toString());
			...
			node.set(IOrgConstants.ORGID_PROPERTY, orgOrganization.getOrgid().toString());
			node.set(IOrgConstants.ORGNAME_PROPERTY, orgOrganization.getOrgname());
	...
	public static DataObject[] buidPositionTreeNodes(OrgPosition[] positions,QueryPositionEmp[] emps){
		List<DataObject> results = new ArrayList<DataObject>();
		results.addAll(Arrays.asList(OrgTreeNodeConvertor.convert(emps)));
		results.addAll(Arrays.asList(OrgTreeNodeConvertor.convert(positions)));
		return results.toArray(new DataObject[results.size()]);
	...
	/**
	 * 将工作组、岗位、员工结点构造为treeNodes
	 * @param orgs
	 * @param positions
	 * @param emps
	 * @return
	 */
	public static DataObject[] buildGroupTreeNodes(OrgGroup[] groups, 
			OrgPosition[] positions, OrgEmployee[] emps) {
		List<DataObject> results = new ArrayList<DataObject>();
		
		results.addAll(Arrays.asList(OrgTreeNodeConvertor.convert(groups)));
		results.addAll(Arrays.asList(OrgTreeNodeConvertor.convert(positions)));
		results.addAll(Arrays.asList(OrgTreeNodeConvertor.convert(emps)));
	...
	public static DataObject[] buildGroupTreeNodes(List<Party> orgPartyList) {
		List<DataObject> results = new ArrayList<DataObject>();
		...
		for(Party orgParty : orgPartyList) {
			...
			node.set(IOrgConstants.GROUPID_PROPERTY, String.valueOf(orgParty.getId()));
			node.set(IOrgConstants.GROUPNAME_PROPERTY, String.valueOf(orgParty.getName()));
			node.set("isLeaf", false);
			node.set("expanded", false);
			results.add(node);
		}
