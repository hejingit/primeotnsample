...
/**
 * 树节点转换器，将机构（工作组），岗位，员工转换成OrgTreeNode对象
...
public class OrgTreeNodeConvertor {
	...
	public static OrgTreeNode[] convert(OrgGroup[] groups){
		if(groups == null || groups.length == 0) return new OrgTreeNode[0];
		OrgTreeNode[] nodes = new OrgTreeNode[groups.length];
		for(int i = 0;i<groups.length;i++){
			nodes[i] = convert(groups[i]);			
		}
		return nodes;
	}
	
	public static OrgTreeNode[] convert(QueryPositionEmp[] posi){
		if(posi == null || posi.length == 0) return new OrgTreeNode[0];
	...
	public static OrgTreeNode convert(OrgGroup group){
		OrgTreeNode node = (OrgTreeNode) DataObjectUtil.convertDataObject(group, OrgTreeNode.QNAME, true);
	...
	public static OrgTreeNode convert(QueryPositionEmp qposi){
		OrgTreeNode node = (OrgTreeNode) DataObjectUtil.convertDataObject(qposi, OrgTreeNode.QNAME, true);
