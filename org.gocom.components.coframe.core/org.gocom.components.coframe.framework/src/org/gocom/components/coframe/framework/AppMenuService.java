 ...
 */
public class AppMenuService extends CoframeDASDaoSupport implements IAppMenuService{
	...
	public void deleteMenuById(String id) {
		...
		AppMenu parent = AppMenu.FACTORY.create();
		ITransactionManager txManager = TransactionManagerFactory
				.getTransactionManager();
		appMenu.setMenuid(id);
		parent=queryParentMenu(id);
		if(parent != null){
			parent.setSubcount(new BigDecimal(parent.getSubcount().intValue()-1));
		}
		txManager.begin(ITransactionDefinition.PROPAGATION_REQUIRED);
		boolean flag=true;
		try{ 
			if(parent!=null){
				updateAppMenu(parent);
			}
			getDASTemplate().deleteEntityCascade(appMenu);
		} catch (Throwable t) {
			flag=false;
	...
	public AppMenu queryParentMenu(String menuId){
		CriteriaType criteria = CriteriaType.FACTORY.create();
		criteria.set_entity(AppMenu.QNAME);
		criteria.set("_expr[1]/menuid",menuId);
		AppMenu[] menu=queryAppMenus(criteria);
		DataObject doj=menu[0].getDataObject(20);
		if(doj!=null){
			criteria.set("_expr[1]/menuid",menu[0].getDataObject(20).getString("menuid"));
		}else{
			return null;
		}
		
		AppMenu[] parent=queryAppMenus(criteria);
		return parent[0];
	}
