...
<script type="text/javascript">
	...
	function removeApplication(){
		...
		if (node.type=='function'){
			message = (isExist(node.realId))?"有菜单与选择的功能关联，删除功能的同时也将删除菜单，继续吗？":"确定删除选择的功能吗？";
		}else if (node.type=='functiongroup'){
			message="功能组下的子功能组、功能，包括与功能关联的菜单都将被删除，继续吗？";
		}else if (node.type=='subfunctiongroup'){
			message="子功能组下的功能，包括与功能关联的菜单都将被删除，继续吗？";
		}else if (node.type=='application'){
			message="应用下的功能组、子功能组、功能，包括与功能关联的菜单都将被删除，继续吗？";
		}else{
			message="该节点下所有子节点都将被删除，确定删除选择的节点吗？";
		}
		
		nui.confirm(message,"删除确认",function(action){
		    if(action!="ok") return;
	        $.ajax({
	...
	function onBeforeOpen(e) {
	    ...
	    if(node.type=="subfunctiongroup"){
	    	var array = [{id: "addfunction", text: "新建功能", iconCls:"icon-add", onclick:"addFunction"},
