...
import org.gocom.components.coframe.framework.auth.FunctionAuthServiceBean;
...
public class EntityCapRuleService implements IEntityCapRuleService{
	...
	private static Logger log = TraceLoggerFactory.getLogger(FunctionAuthServiceBean.class);
	 ...
	 */
	public synchronized String deleteCapRule(Rule[] rules){
		...
		if(rules != null) {
			...
			try{
				for(Rule rule : rules) {
					...
					if(partyList != null && partyList.size() !=0) {
						for(Party party : partyList) {
							isSuccess= isSuccess && AuthRuntimeManager.getInstance().delAuthRes(party, authResource, IAuthManagerService.DEL_MODE_SINGLE);
							if(!isSuccess)
								break;
						}
					}
				...
				log.error("Delete CAP Role failure, please do the operation again or contact the sysadmin.", t);
				isSuccess=false;
			}finally{
				if(isSuccess){  
					txManager.commit();
					...
					for(Rule rule:rules){
						RuleService.INSTANCE.deleteRuleCache(rule.getId());
					}
					result=FAIL;
				}
			}
     ...
     */
    public synchronized String updateCapRule(String ruleId, String ruleName, Condition[] ruleExpression, Party party){
    	String result=SUCCESS;
    	if(isUniqueRuleName(ruleId, ruleName, UPDATE_RULE)) {
    		...
    		if(rules != null){
    			...
    			try{
    				...
    				log.error("Update CAP Role failure, please do the operation again or contact the sysadmin.", t);
    				isSuccess=false;
    			}finally{
    				if(isSuccess){
    					txManager.commit();
    					...
    					txManager.rollback();
    					for(int i=0; i < rules.length; i++) {
    						RuleService.INSTANCE.deleteRuleCache(rules[i].getId());
            			}
    					result=FAIL;
    				}
    			}
