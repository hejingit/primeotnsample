		...
		for (IManagedResource resource : resources) {
			...
			if (StringUtil.isNotEmpty(funcAction)) {
				...
				if (StringUtil.equal(tenantId, funcTenant)
					...
					String urlParam=StringUtil.substringAfter(funcAction, "?");
					if (funcParam == null && urlParam==null) {
						return resource;
					} else {
						if (funcParam != null && isContain(paramMap, funcParam)) {
						     return resource;
					    }else if(urlParam != null && isContain(paramMap, urlParam)){
					    	return resource;
					    }
					}
				}
		...
		for (int i = 0; i < funcParams.length; i++) {
			if (!StringUtil.isBlank(funcParams[i])) {
				...
				if (!StringUtil.isBlank(params[0])) {
					...
					String paramValue = ((String[]) paramMap.get(params[0]))[0];
