 ...
 */
public class DefaultRoleManager extends CoframeDASDaoSupport {
	
	private static Logger log = TraceLoggerFactory.getLogger(DefaultRoleManager.class);
	...
	public QueryGroupByRole[] queryGroupsByRoleId(String roleId, String tenantID) {
		IDASCriteria criteria = DASManager.createCriteria(QueryGroupByRole.QNAME);
		criteria.add(ExpressionHelper.eq(IConstants.TENANT_PROPERTY, tenantID));
		criteria.add(ExpressionHelper.eq("roleId", roleId));
		criteria.add(ExpressionHelper.eq("partyType", IConstants.GROUP_PARTY_TYPE_ID));
		return getDASTemplate().queryEntitiesByCriteriaEntity(QueryGroupByRole.class, criteria);
	}
