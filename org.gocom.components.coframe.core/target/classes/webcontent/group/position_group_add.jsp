...
<body>
<div class="nui-fit" style="padding-top:5px;min-width:450px;min-height:70px;">
	<div id="form1" style="width:100%;height:99%;overflow:hidden;">
		<input class="nui-hidden" name="groupposi.orgGroup.groupid" />
		<table style="width:100%;height:100%;" class="nui-form-table" >
			<tr>
				<th class="nui-form-label"><label for="posiname$text">岗位名称：</label></th>
				<td><input id="posiname" class="nui-textbox nui-form-input" name="groupposi.orgPosition.posiname" required="true" vtype="maxLength:128"/></td>
				<th class="nui-form-label"><label for="posicode$text">岗位代码：</label></th>
				<td><input id="posicode" class="nui-textbox nui-form-input" name="groupposi.orgPosition.posicode" required="true" vtype="maxLength:20"/></td>
			</tr>		
			<tr class="odd">
				<!-- th class="nui-form-label"><label for="orgname$text">所属机构：</label></th>
				<td>
				 <input id="orgname" class="nui-textbox nui-form-input" name="groupposi.orgPosition.orgOrganization.orgname" allowInput="false" />
				</td -->
				<th class="nui-form-label"><label for="startdate$text">生效日期：</label></th>
				<td>
				<input id="startdate" name="groupposi.orgPosition.startdate" class="nui-datepickerx nui-form-input" />
				</td>
				<th class="nui-form-label"><label for="enddate$text">失效日期：</label></th>
				<td><input id="enddate" name="groupposi.orgPosition.enddate" class="nui-datepickerx nui-form-input" onvalidation="onEnddateValidation" />
				</td>
			</tr>				
		</table>
	</div>
...
<script type="text/javascript">
	...
	function add(){
       ...
       var o = form.getData();
        ...
        $.ajax({
            url: "org.gocom.components.coframe.org.groupposi.addGroupposi.biz.ext",
            type: 'POST',
            data: json,
            cache: false,
	...
	function SetData(data){
		...
		form.setData({groupposi:{orgGroup:data.parentNode}});
	}
	
	//失效日期必须大于生效日期
	function onEnddateValidation(e){
       	var o = form.getData();
       	var org = o.groupposi.orgPosition || {};
